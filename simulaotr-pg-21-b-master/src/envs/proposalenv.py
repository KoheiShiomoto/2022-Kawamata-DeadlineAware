...
class ProposalEnv(environment.Env):
    def __init__(self,link_utilization,act_num,bandwith_distribution,file_path,file_parameter_name):
        ...
        self.act_now=False # ジョブ選択を行うかどうかのbool値
    ...
    def step(self,action):
        ...
        #ジョブの処理
        if (len(self.buffer)!=0)and(action<self.wait_num): # ジョブがあり，action<Nact(ジョブを選択した場合)
            self.act_now=True
            ...
            if self.bandwith_distribution==None:
                ...
                high_bandwith=rd.uniform(0,0.2)
                self.buffer[action][0]-=self.decrease_job*(1.0-high_bandwith)
            else:
            ...
            if self.buffer[action][0]<=0:
                ...
                self.act_now=False # ジョブ選択を行えるようにする
    ...
    def get_value(self,completed_job_id):
        ...
        if completed_job_id is None:
            ...
            # 遅延の計算
            delay=min(0,self.job_log[completed_job_id][0]-self.job_log[completed_job_id][-1])
            value=np.exp(delay*self.reduction_rate)
